...
# data1 row; data2 col
score_matrix<-function(data1,data2,parallel=F,cores = 10,...){
  type = 'local'
  if(parallel){
    temp_func<-function(x,y){pairwiseAlignment(subject = AAStringSet(x), pattern = AAStringSet(y),
                                                     substitutionMatrix = 'BLOSUM62',
                                                     scoreOnly=T,type = type,...)}
    # print(data1[1:5])
    # print(data2[1:5])
    result = t(mcmapply(temp_func, data1, rep(list(data2), length(data1)), mc.cores = cores))

  }else{
    result = outer(data1,data2, function(x,y) pairwiseAlignment(AAStringSet(x) ,AAStringSet(y),
                                                     substitutionMatrix = 'BLOSUM62',
                                                     scoreOnly=T,type = type,...))
  }
  return(result)
...
training<-function(data1, data2 = NULL, threshold = NULL,
  ...
  cv_result = data.frame(pep = data2[,1], score = data2[,2], cv_est = kernel_est)
...
training_fold<-function(data1, k_fold, threshold = NULL, grid_len = 200,
                   plot.cv = F, K = dnorm, 
                   only_config = F, seed = NULL,
                   parallel=F, cores = 10){
  ...
  # min(self_scaled(matrix)) = 0; max(self_scaled(matrix)) = 1
  h.grid = seq(0.15,0.85,length.out = grid_len)
  ...
  if(parallel){
      obj <- mclapply(h.grid, get_coe, mc.cores = cores)
      obj <- Reduce(c, obj)
    }else{
...
pred<-function(data1, data2, k_fold = NULL,threshold = NULL, config=NULL,K = dnorm,
  ...
  temp_matrix = score_matrix(data1[,1], data2, parallel=parallel, cores = cores)
  temp_matrix = 1 - pred_scaled(temp_matrix,config$tmax,config$tmin)
  temp_matrix[temp_matrix<0] = 0
  kernel_est = mNW(temp_matrix, Y = data1[,2], h=config$bw,cv=F,K = K)
...
epitope_region<-function(data1, data2, threshold = NULL, config=NULL, k_fold = NULL,
  ...
  final_result = data.frame(pep = pep, res = seq_array, pred = prediction, stringsAsFactors = F)
