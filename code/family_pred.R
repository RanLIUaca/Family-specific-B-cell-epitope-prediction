...
for (i in 1:n) {
  ...
  temp_virus_data = temp_virus_data[sel_criterion,][,c('Sequence', 'Response.Freq.','Virus')]
  temp_virus_data_nr = temp_virus_data[!duplicated(temp_virus_data$Sequence), ]
  print(nrow(temp_virus_data)-nrow(temp_virus_data_nr))
  ...
  if(nrow(temp_virus_data[temp_virus_data$Response.Freq.>0,])<=30){
    ...
    raw_data = c(raw_data, list(temp_virus_data_nr))
  }
}
...
for (j in 1:length(data_names)) {
  colnames(raw_data[[j]]) = c('pep','prob','virus')
}

total_result = Reduce(rbind, raw_data)
new_result = total_result[!duplicated(total_result$pep), ]
raw_data = list()
for (i in 1:n) {
  temp_data = data1[data1['Virus']==data_names[i],]
...
all_threshold = 'family_pred'
possible_thresholds = 5

# all pred
# all_threshold = 'all_pred'
# possible_thresholds = 13
